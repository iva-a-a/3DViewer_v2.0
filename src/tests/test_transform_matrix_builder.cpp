...
TEST_F(TransformMatrixBuilderTest, CreateRotateMatrix) {
  // Угол вращения
  float a = 0.0f, b = 0.0f,
        c = 3.14159265359f;  // Поворот на 180 градусов вокруг оси Z

  s21::TransformMatrix result =
      s21::TransformMatrixBuilder::createRotateMatrix(a, b, c);

  // Проверяем значения в матрице
  EXPECT_FLOAT_EQ(result(0, 0), -1.0f);  // Элемент на позиции (0, 0) должен
                                         // быть -1 для поворота на 180 градусов
  EXPECT_FLOAT_EQ(result(1, 1), -1.0f);  // Элемент на позиции (1, 1) должен
                                         // быть -1 для поворота на 180 градусов
  EXPECT_FLOAT_EQ(result(2, 2),
                  1.0f);  // Элемент на позиции (2, 2) должен быть 1
}
...
TEST_F(TransformMatrixBuilderTest, CreateMoveMatrix) {
  ...
  EXPECT_FLOAT_EQ(result(3, 0),
                  1.0f);  // Элемент на позиции (0, 3) должен быть 1
  EXPECT_FLOAT_EQ(result(3, 1),
                  2.0f);  // Элемент на позиции (1, 3) должен быть 2
  EXPECT_FLOAT_EQ(result(3, 2),
                  3.0f);  // Элемент на позиции (2, 3) должен быть 3
}
...
TEST_F(TransformMatrixBuilderTest, CreateScaleMatrix) {
  ...
  EXPECT_FLOAT_EQ(result(0, 0),
                  2.0f);  // Элемент на позиции (0, 0) должен быть 2
  EXPECT_FLOAT_EQ(result(1, 1),
                  3.0f);  // Элемент на позиции (1, 1) должен быть 3
  EXPECT_FLOAT_EQ(result(2, 2),
                  4.0f);  // Элемент на позиции (2, 2) должен быть 4
}
