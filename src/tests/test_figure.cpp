...
TEST_F(FigureTest, Constructor) {
    ...
    ASSERT_EQ(figure.getVertices().size(), 8);  // В файле cube.obj должно быть 8 вершин
    ASSERT_EQ(figure.getFacets().size(), 36);  // В файле cube.obj должно быть 36 граней
}
...
TEST_F(FigureTest, CopyConstructor) {
    ...
    // Проверяем, что количество вершин и граней одинаково в оригинальном и копированном объекте
...
TEST_F(FigureTest, AssignmentOperator) {
    ...
    // Проверяем, что количество вершин и граней одинаково в оригинальном и присвоенном объекте
...
TEST_F(FigureTest, Normalization) {
    ...
    // Применяется ли нормализация? (Проверим, что вершины не равны нулевым значениям)
    ...
    ASSERT_TRUE(normalizationApplied); // Ожидаем, что нормализация вершин была применена
}
...
TEST_F(FigureTest, GetVerticesAndFacets) {
    ...
    EXPECT_EQ(facets[0].getBeginPosition()->x(), 1.0); // Проверка первого ребра
    EXPECT_EQ(facets[0].getEndPosition()->x(), -1.0); // Проверка первого ребра
}
